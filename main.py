...
from pymisp import MISPObject, MISPTag
...
# Defined below as per:https://pymisp.readthedocs.io/en/latest/_modules/pymisp/api.html
MISP_URL = ""
MISP_KEY = ""
...
def misp_create(extracted_data, inputfilename, filemd5sum):
    misp = pymisp.PyMISP(MISP_URL, MISP_KEY, MISP_VERIFYCERT)

    event = pymisp.MISPEvent()
    event.info = "Contileaks: " + inputfilename

    event_tag = MISPTag()
    event_tag.from_dict(name='Contileaks')
    event.add_tag(event_tag)

    misp_object = MISPObject('file')
    misp_object.add_attribute('filename', type='filename',
                              value=inputfilename)
    misp_object.add_attribute('md5', type='md5', value=filemd5sum)
    event.add_object(misp_object)

    for key in extracted_data:
        for value in extracted_data[key]:
            if key == "email":
                misp_object_data = MISPObject(key)
                misp_object_data.add_attribute(key, type=key, value=value)
                event.add_object(misp_object_data)
            if key == "ipv4":
                misp_object_data = MISPObject('url')
                misp_object_data.add_attribute('ip', type='ip-dst', value=value)
                event.add_object(misp_object_data)
            if key == "btc":
                misp_object_data = MISPObject('coin-address')
                misp_object_data.add_attribute('address', type='btc', value=value)
                event.add_object(misp_object_data)
...
if __name__ == '__main__':
    ...
    for inputfilename in files:
        ...
        print("Creating misp events for: ", inputfilename)
        # Create misp events for extracted data
        misp_create(extracted_data, inputfilename, filemd5sum)
